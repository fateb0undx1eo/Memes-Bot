...
def setup_cog_directory():
    ...
    if not os.path.exists(init_file):
        ...
        logger.info(f"Created __init__.py in {COGS_DIR}")
...
async def load_all_cogs():
    ...
    for filename in os.listdir(COGS_DIR):
        if filename.endswith(".py") and not filename.startswith("_"):
            ...
            try:
                # Unload if already loaded (for reloading)
                ...
                logger.error(f"Error details: {''.join(traceback.format_exception(type(e), e, e.__traceback__))}")
            except Exception as e:
...
async def main():
    ...
    async with bot:
        # Start the bot - cogs will be loaded in on_ready
...
if __name__ == "__main__":
    ...
    # Enhanced error handling for cog loading
    try:
        asyncio.run(main())
        ...
        logger.error(traceback.format_exc())    "memes", "dankmemes", "funny", "me_irl",
    "animememes", "goodanimemes", "wholesomememes",
    "AdviceAnimals", "MemeEconomy", "terriblefacebookmemes",
    "nsfwmemes", "dankmemes_nsfw", "EdgyMemes", "darkmemers"
]
